...
function addEventsForAnswers() {
  ...
  Array.from(answersRef).forEach((answer) => {
    ...
    radioBtnRef.addEventListener("click", () => {
      ...
      !questionResultMap.has(questionNumber)
        ? (answeredCounterRef.innerHTML = ++answeredNumber + "/10")
        : "";
...
function createResultTable() {
  for (let i = 0; i < QUESTIONS_AMOUNT; i++) {
    ...
    statusImg.setAttribute(
      "class",
      checkCorrectAnswer(i) ? "question-status-true" : "question-status-false"
    );
...
function checkCorrectAnswer(questionNumber) {
  if (questionResultMap.has(questionNumber + 1)) {
    ...
    return (
      ...
      questionResultMap.get(questionNumber + 1)
    );
  }
  return false;
}
...
function showHint() {
  hintBlockRef.children[1].innerText =
    currentQuestions[questionNumber - 1].hintText;
